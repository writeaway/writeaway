...
export const getNodeRect = (piece: IPiece, padding: number = 10) => {
  ...
  if (
    hover.left + window.scrollX < 0
    || hover.top + window.scrollY < 0
    || hover.width + hover.left + window.scrollX > document.body.scrollWidth
    || hover.height + hover.top + window.scrollY > document.body.scrollHeight
  ) {
    return {
      node,
    };
